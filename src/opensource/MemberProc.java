...
public class MemberProc extends JFrame implements ActionListener {
    ...
    public void actionPerformed(ActionEvent ae) {
 	   String id = tfId.getText(); //id
       String pwd = pfPwd.getText(); //pw
 

       if(Pattern.matches("^(?=.*[0-9])(?=.*[a-z])(?=.*[a-zA-Z])(?=.*[!,@,#,$,%,^,&,*,?,_,~]).{10,25}$",  pwd)){ 
	    	insertMember();
     	 
	    }
	    else  {
	    	 JOptionPane.showMessageDialog(this, "문자, 숫자, 특수문자를 포함하여 10자 이상 입력하세요");
	       	 return;
	       } 
    
       String listThreeChar = "abc|bcd|cde|def|efg|fgh|ghi|hij|ijk|klm|lmn|mno|nop|opq|qrs|rst|stu|tuv|uvw|vwx|wxy|xyz|012|123|234|345|456|567|678|789|890";
    	String[] arrThreeChar = listThreeChar.split("\\|");
    			for(int i=0; i<arrThreeChar.length;i++){
    			 if(pwd.toLowerCase().matches(".*"+arrThreeChar[i]+".*")){
    				 JOptionPane.showMessageDialog(this, "연속된 3자리 이상의 문자나 숫자가 포함되어있습니다.");
    		       	 return;
    			 }else{
    				 insertMember();
    			 }
